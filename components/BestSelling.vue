  ...
  <div class="px-2">
    ...
    <vue-horizontal ref="horizontal" class="horizontal" responsive v-if="products">
        ...
        <div v-for="product in products" :key="product.id" class="flex flex-row px-5 mb-10">
          ...
          <!-- <a :href="$router.resolve({path: 'Product', params: { id: product.id, slug: sanitizeTitle(`${product.name}`) }}).href" class="w-60 h-68"> -->
            {{}}
            <!-- <NuxtLink :to="{params: { id: product.id, slug: sanitizeTitle(`${product.name}`) }}" class="w-60 h-68"> -->
